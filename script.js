...
function getDisplayedProducts() {
  ...
  const visible = reformattedElements.filter((element) => {
    return (
      element.coordinates.top < window.innerHeight &&
      element.coordinates.bottom > 0 &&
      !loggedProducts.includes(element.title)
    );
  });
  ...
  function (entries, observer) {
    ...
    entries.forEach((entry) => {
      ...
      if (entry.isIntersecting) {
        const title = entry.target.querySelector(
          ".ProductCard__title"
        ).innerText;
        const price = entry.target.querySelector(
          ".ProductCard__price"
        ).innerText;
